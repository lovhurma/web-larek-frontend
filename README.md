...
- card:select - Инициируется при клике на карточку товара и получает данные о ней - объект конкретного товара
- prepreview:change - Инициируется при передаче моделью новых данных, оповещает View, приводит к отрисовке содержимого модального окна 
...
- order:toContacts - Инициируется при клике на кнопку "Оформить". Далее приводит к открытию модального окна с заполнением контактов
- input:change - Инициируется при вводе данных в форму заказа Order и контактов ContactsЗапускает процесс валидации
