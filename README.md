...
- basket:remove - Инициируется при клике на значек корзины внутри самой корзины, вызывает перерисовку списка товаров и счетчика на главной страничце
- basket:order - Инициируется при клике на кнопку "Оформить" в корзине, приводит к открытию модального окна с выбором способа оплаты и адреса доставки
...
- error:changed -  совершает процесс валидации формы, возвращает ошибки формы
...
- getItems() - Метод для получения списка карточек 
...
- getBasketId() - Метод для получения списка ID товаров в корзине
- getBasket() - Метод для получения массива карточек в корзине
- getField() - Метод для получения поля оплаты заказа
- getOrder() - Метод возвращающий весь объект заказа
...
Отвечает за отображение данных карточки товара в корзине, в модальном окне отдельной карточки и в каталоге корзины. Поля отвечают за связь с разметкой, методы за наполнение разметки данными.
Рассширяет базовый абстрактный класс Component по интерфейсу IProduct
...
- _index? - хранит разметку порядкового номера товара в заказе

Методы класса
- set id - устанавливает id карточки
- set price - устанавливает стоимость карточки
- set title - устанавливает заголовок карточки
- set button - устанавливает заголовок кнопки
- set index - устанавливает порядковый номер карточки в корзине

#### Класс CardCatalog

Отвечает за отображение данных карточки товара в каталоге. Поля отвечают за связь с разметкой, методы за наполнение разметки данными. Рассширяет Card 
...
Отвечает за отображение данных превью товара в каталоге. Поля отвечают за связь с разметкой, методы за наполнение разметки данными. Рассширяет CardCatalog 
